...
**数据字典内容必须严格与数据集字段一致：**
- 字段英文名必须与数据集中的列名完全一致，不得有任何修改、拼写变化或格式调整
- 如果数据集中的列名是中文，字段中文名必须使用完全相同的中文列名
- 如果数据集中的列名是英文，字段英文名必须使用完全相同的英文列名
- 字段中文名和字段英文名必须严格反映原始字段名，不得添加、删除或更改任何字符
- 不得对字段名进行任何形式的翻译、转写、缩写或扩展
- 不得对字段名的大小写、空格、下划线、连字符等格式进行任何修改
- 数据字典中的字段顺序必须与数据集中的列顺序完全一致
